...
The original code only supported multitrack with a single set of fixed ADSR parameters.
---
...
from tone import Tone, Samples, SINE_WAVE, NUM_CHANNELS, DATA_SIZE
...
class Track(object):
    ...
    def __init__(self, wavetype=SINE_WAVE, attack=None, decay=None,
            sustain=None, release=None, gain=1.0, vibrato_frequency=None, vibrato_variance=15.0):
        """
        Initializes a Track
        ...
        :param float decay: initial tone decay for this track, to applied to \
            each set of samples generated by 'append_samples'
        :param float sustain: initial tone sustain level for this track
        :param float release: initial tone release time for this track
        :param float gain: track gain/volume multiplier (default 1.0 = 100%)
...
class Mixer(object):
    ...
    def add_tone(self, trackname, frequency=440.0, duration=1.0,
            endfrequency=None, attack=None, decay=None, sustain=None,
            release=None, amplitude=1.0, vibrato_frequency=None,
            vibrato_variance=None):
        """
        ...
        :param float decay: tone decay in seconds. Overrides the track's \
            decay setting
        :param float sustain: tone sustain level (0.0-1.0). Overrides the track's \
            sustain setting
        :param float release: tone release time in seconds. Overrides the track's \
            release setting
        ...
        if not decay:
            decay = track._decay
        if sustain is None:
            sustain = track._sustain
        if release is None:
            release = track._release
        ...
        samples, track._phase, track._vphase = tone.samples(numsamples,
                frequency, endfrequency, attack, decay, sustain, release,
                track._phase, track._vphase, vibrato_frequency, vibrato_variance)
    ...
    def mix(self):
        ...
        for track in tracks:
            for i in range(len(track._samples)):
                ret[i] += (track._samples[i] * weight * track._gain) * self._amp
    ...
    def write_wav(self, filename, sampledata=None):
        ...
        f.setparams((NUM_CHANNELS, DATA_SIZE, self._rate,
